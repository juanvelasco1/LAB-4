        ...
        `;
			...
			episodesData.forEach((ep: any) => {
				...
				ep.characters.forEach(async (character: string) => {
					...
					const newCharacter = this.ownerDocument.createElement('my-character') as Character;
					newCharacter.setAttribute(AttributeChar.Origin, await getCharacters(character));
